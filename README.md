...
- **Automatically click "Load Queue" when the current queue is empty**: When you mark an item complete and the extension would open the next submission, but the queue is empty, it automatically clicks the `Load Queue` button to fetch more submissions and then opens the next item.
- **Click "Load Queue" every hour when the queue has less than 10 items**: Starts a one-hour timer on page load (when enabled). Each time the timer fires, the extension counts visible queue items and clicks `Load Queue` only when there are fewer than 10. If there are already 10 or more items, it waits another hour and checks again. The timer is reset whenever `Load Queue` is clicked, whether clicked manually or automatically. Changing this setting on/off while the queue page is open immediately starts or clears the timer.

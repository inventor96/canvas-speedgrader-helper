(() => {
  ...
  // ============================================================================
  // NAMESPACE: NameSanityCheck
  // Detects unnatural name formats (all-uppercase/all-lowercase) and offers
  // to save a properly formatted preferred name.
  // ============================================================================
  const NameSanityCheck = {
    ...
    check() {
      try {
        const params = new URLSearchParams(location.search || window.location.search);
        const sid = params.get('student_id');
        // Skip if a preferred name is already saved for this student
  ...
  // Check if the current student name is in an unnatural format (all-uppercase
  // or all-lowercase) and offer to save a preferred name.
  if (ENABLE_NAME_SANITY_CHECK) {
    try {
      setTimeout(() => {
        const tryCheck = (retry = 0) => {
          NameSanityCheck.check();
          // Re-check once if the name wasn't available yet (up to 20s)
